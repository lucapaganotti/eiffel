note
	description : "rt10pg application root class"
	date        : "$Date$"
	revision    : "$Revision$"
...
feature {NONE} -- Initialization
	...
	init
		...
		do
			...
			-- ODBC
			create dsn.make_empty
			create dsn_usr.make_empty
			create dsn_pwd.make_empty
			create dsn_schema.make_empty
	...
	make
		...
		do
			...
			set_data_source(dsn.twin)
			set_hostname ("localhost")
			login (dsn_usr, dsn_pwd)
			set_application (dsn_schema.twin)
			set_base
...
feature -- Process
	...
	make_last_dates_queries (sensor: RT10_SENSOR) : TUPLE[STRING, STRING, STRING]
		...
		do
			...
			loop
				...
				query := "select data_e_ora from " + dsn_real_time_schema + ".m_osservazioni_tr where idsensore = " + sensor.sensor_id.out
	...
	make_insert_into_data (s: INTEGER; o: INTEGER; t: STRING; m: MEASURE) : STRING
		...
		do
			...
			Result := "INSERT INTO "+ dsn_real_time_schema + ".m_osservazioni_tr " +
                      "(idsensore, " +
                      "nometipologia, " +
                      "idoperatore, " +
                      "data_e_ora, " +
	...
	make_exists_measure_query (s: INTEGER; o: INTEGER; t: STRING; d: DATE_TIME) : STRING
		...
		do
			Result := "select data_e_ora from " + dsn_real_time_schema + ".m_osservazioni_tr where "+
	...
	load_sensors(some_results: ARRAYED_LIST[DB_RESULT])
		...
		do
			if session_control.is_connected then
				...
				io.put_string ("Connected through ODBC to " + dsn + " DSN%N")

				-- chiedere l'elenco dei sensori

				selection.set_query ("select idsensore, frequenza, nometipologia from " + dsn_base_data_schema + ".anagraficasensori order by idsensore desc;")
				selection.execute_query
...
feature -- Operations
	...
	post(msg: STRING): STRING
		...
		do
			...
			if curl_easy.is_dynamic_library_exists then
				...
				curl_easy.setopt_string  (curl_handle, {CURL_OPT_CONSTANTS}.curlopt_useragent,     "RT10PG curl testclient")
	...
	delete_old_data
		...
		do
			...
			l_query := "delete from " + dsn_real_time_schema + ".m_osservazioni_tr where data_e_ora < '" + l_date.formatted_out (default_date_time_format) + "';"
...
feature -- Preferences
	...
	dsn_pref:                  STRING_PREFERENCE
			-- Data Source Name preference
	dsn_usr_pref:              STRING_PREFERENCE
			-- DSN user preference
	dsn_pwd_pref:              STRING_PREFERENCE
			-- DSN password preference
	dsn_schema_pref:           STRING_PREFERENCE
			-- DSN schema preference
	dsn_base_data_schema_pref: STRING_PREFERENCE
			-- PG base data schema
	dsn_real_time_schema_pref: STRING_PREFERENCE
			-- PG real time data schema
	...
	init_preferences
			-- Loads preferences
		do
			create preferences_storage.make_with_location (preferences_folder + "/rt10pgconf.xml")
			create preferences.make_with_storage (preferences_storage)
			preference_manager := preferences.new_manager ("rt10pg")

			create factory
			host_pref                 := factory.new_string_preference_value (preference_manager,  "rt10pg.host",                 "localhost")
			database_pref             := factory.new_string_preference_value (preference_manager,  "rt10pg.database",             "METEO")
			dbusr_pref                := factory.new_string_preference_value (preference_manager,  "rt10pg.dbusr",                "root")
			dbpwd_pref                := factory.new_string_preference_value (preference_manager,  "rt10pg.dbpwd",                "METEO")
			collect_host_pref         := factory.new_string_preference_value (preference_manager,  "rt10pg.collect_host",         "localhost")
			collect_port_pref         := factory.new_integer_preference_value (preference_manager, "rt10pg.collect_port",         9090)
			dsn_pref                  := factory.new_string_preference_value (preference_manager,  "rt10pg.dsn",                  "local")
			dsn_usr_pref              := factory.new_string_preference_value (preference_manager,  "rt10pg.dsn_usr",              "buck")
			dsn_pwd_pref              := factory.new_string_preference_value (preference_manager,  "rt10pg.dsn_pwd",              "chi66rone;")
			dsn_schema_pref           := factory.new_string_preference_value (preference_manager,  "rt10pg.dsn_schema",           "buck")
			dsn_base_data_schema_pref := factory.new_string_preference_value (preference_manager,  "rt10pg.dsn_base_data_schema", "public")
			dsn_real_time_schema_pref := factory.new_string_preference_value (preference_manager,  "rt10pg.dsn_real_time_schema", "public")
			...
			dsn                       := dsn_pref.value
			dsn_usr                   := dsn_usr_pref.value
			dsn_pwd                   := dsn_pwd_pref.value
			dsn_schema                := dsn_schema_pref.value
			dsn_base_data_schema      := dsn_base_data_schema_pref.value
			dsn_real_time_schema      := dsn_real_time_schema_pref.value
			...
			display_line ("DSN:                  " + dsn,                  true)
			display_line ("DSN user:             " + dsn_usr,              true)
			display_line ("DSN password:         " + "*******",            true)
			display_line ("DSN schema:           " + dsn_schema,           true)
			display_line ("DSN base data schema: " + dsn_base_data_schema, true)
			display_line ("DSN real time schema: " + dsn_real_time_schema, true)
...
feature -- ODBC
	...
	dsn_pwd:              STRING
	dsn_schema:           STRING
	dsn_base_data_schema: STRING
	dsn_real_time_schema: STRING
	session_2:            DATABASE_SESSION
	...
	preferences_folder: STRING_32
		...
		once
			if( home_folder.is_empty ) then
				create Result.make_from_string("./.rt10pg")
			else
				create Result.make_from_string(home_folder + "/.rt10pg")
			end
