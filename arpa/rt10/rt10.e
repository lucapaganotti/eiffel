...
feature {NONE} -- Initialization
	...
	init
		...
		do
			...
			create realtime_data_meteo_res.make
		...
		do
			...
			loop
				...
				if attached response as res and not response.is_empty then
					-- **************** meteo ********************
					realtime_data_meteo_res.sensor_data_list.wipe_out
					realtime_data_meteo_res.from_json (res, json_parser)
					...
					-- io.put_new_line
					-- **************** meteo ********************
					if realtime_data_meteo_res.sensor_data_list.count = 0 then
...
--					end
					...
					-- **************** meteo ********************
					from realtime_data_meteo_res.sensor_data_list.start
					until realtime_data_meteo_res.sensor_data_list.after
					loop
						--display_line (realtime_data_res.sensor_data_list.i_th (j).out + " " + sensor.operators.i_th (sensor.last_dates.index).out, true)
						--display_line (realtime_data_res.sensor_data_list.item.out + " " + sensor.operators.i_th (sensor.last_dates.index).out, true)
						from realtime_data_meteo_res.sensor_data_list.item.data.start
						until realtime_data_meteo_res.sensor_data_list.item.data.after
						loop
							l_line := realtime_data_meteo_res.sensor_data_list.item.data.item.out
							...
							realtime_data_meteo_res.sensor_data_list.item.data.forth
						end
						realtime_data_meteo_res.sensor_data_list.forth
					end
					-- **************** meteo ********************
...
feature -- Messages
	...
	realtime_data_meteo_res: REALTIME_DATA_METEO_RESPONSE
			-- Data meteo response
