	note
	description : "Summary description for {REALTIME_DATA_METEO_REQUEST}."
...
--| {
--|   "header": {
--|     "id":  <realtime_data_meteo_request_id=13>
--|   },
--|   "data": {
--|     "sensors_list": [{
--|                        "sensor_id": id1,
...
feature {NONE} -- Initialization
	...
	make_from_token (a_token: STRING)
			-- Build a `REALTIME_DATA_METEO_REQUEST' with `token_id' = `a_token'
		do
...
feature -- Access
	...
	id: INTEGER
			-- message id
		do
			Result := realtime_data_meteo_request_id
		end
...
feature -- Basic operations
	...
	init_response: detachable RESPONSE_I
			--
		do
			Result := create {REALTIME_DATA_METEO_RESPONSE}.make
		end
...
feature {NONE} -- Utilities implementation
	...
	name: STRING
		...
		do
			Result := realtime_data_meteo_endpoint_name
		end
...
--	        	  <s:Body>
--	        	    <RendiDatiTempoRealeMeteo xmlns="http://tempuri.org/">
--	       	          <xInput>
--	                    <RendiDatiTempoRealeMeteo xmlns="">
--	                      <Token>
--	                        <Id>$tokenid</Id>
--	                      </Token>
--	                      $sensors
--	                    </RendiDatiTempoRealeMeteo>
--	                  </xInput>
--	                </RendiDatiTempoRealeMeteo>
--	              </s:Body>
--	            </s:Envelope>
--			]"
			...
			Result.append (stag_start + xmlns_s + colon + soap_envelope + space + xmlns + colon + xmlns_s + eq_s + double_quotes + xmlsoap + double_quotes + stag_end + lf_s)
			  Result.append (double_space + stag_start + xmlns_s + colon + body + stag_end + lf_s)
			    Result.append (double_space + double_space + stag_start + realtime_data_meteo_endpoint_name + space + xmlns + eq_s + double_quotes + remws_uri + url_path_separator + double_quotes + stag_end + lf_s)
			      Result.append (double_space + double_space + double_space + stag_start + xinput + stag_end + lf_s)
			        Result.append (double_space + double_space + double_space + double_space + stag_start + realtime_data_meteo_endpoint_name + space +
			        ...
			        Result.append (double_space + double_space + double_space + double_space + etag_start + realtime_data_meteo_endpoint_name + etag_end + lf_s)
			      Result.append (double_space + double_space + double_space + etag_start + xinput + etag_end + lf_s)
			    Result.append (double_space + double_space + etag_start + realtime_data_meteo_endpoint_name + etag_end + lf_s)
